...
// build.mjs（Top1500 精选）与 build-ext.mjs（全量扩展分片）共用，保证派生口径完全一致。

export const ALPHA = 5; // 季节拉普拉斯平滑系数
 ...
 * 讽刺的是，仓库特意放宽汉字品牌的票数门槛以求包容，却基于零数据替这些国货编造了留香与扩散。
 *
...
// 原始精简记录（extract-terms 的 rec 形状）+ 中文映射 → Perfume 形状（与 src/lib/types.ts 同构）
// maps: { accordMap, brandMap, noteMap, nameMap }；缺映射时回退英文 / null / []
export function toPerfume(r, maps) {
  const { accordMap = {}, brandMap = {}, noteMap = {}, nameMap = {} } = maps;
