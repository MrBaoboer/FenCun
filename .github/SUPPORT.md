# 获取帮助 / Support

> English version: [SUPPORT_EN.md](SUPPORT_EN.md)

先对照下面这张表找对地方，能最快得到回应。

## 我该去哪

| 你的情况 | 去哪 |
|---|---|
| 想了解氛寸是什么、怎么用 | 先看 [README](../README.md) 与在线体验 [fencun.vercel.app](https://fencun.vercel.app) |
| 想读完整产品思路 / 路线图 | [产品方案](../docs/氛寸-产品方案.md)（路线图在其中，本项目不单独维护 ROADMAP） |
| 发现 Bug | 开一个 [Bug 报告](https://github.com/MasterBao66/FenCun/issues/new?template=bug_report.yml) |
| 有功能建议 | 先确认与产品定位一致，再开 [功能建议](https://github.com/MasterBao66/FenCun/issues/new?template=feature_request.yml) |
| 想动手贡献 | 读 [贡献指南](CONTRIBUTING.md)——尤其是四条戒律与治理 |
| 发现安全漏洞 | **不要开公开 Issue**，走 [安全政策](SECURITY.md) 私密报告 |

## 提问前请附上（Web 应用）

- 浏览器与版本、设备（如 Chrome 141 · iPhone 15）
- 复现步骤，或一个能复现的链接
- 出问题的页面（今日 / 香柜 / 我的）
- 期望的结果 vs 实际结果

## 关于响应

氛寸由独立开发者单人维护，**不承诺固定响应时间**；优先级大致是：安全 > 可复现的 Bug > 清晰的 PR > 功能建议。联系方式见维护者 [@MasterBao66](https://github.com/MasterBao66) 主页公开的邮箱。
