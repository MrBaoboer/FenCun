...
export function weatherFit(p: Perfume, feel: Feel, tempC?: number, humidity?: number): WeatherResult {
  ...
  // 闷指数。**乘 H 而不是被 H 门控**——写成 `H > 0 ? … : 0` 时它不随 H 连续趋零，
  // 而是在 t 越过 22.0 的瞬间从 0 跳到满值：实测 90%RH 下 22.00→22.01℃，
  // 天气乘子从 1.0000 掉到 0.9399（6%），全目录 219 款（14.6%）从 good 翻成 caution，
  // 配的文案还是「今天气温偏高」——22℃ 谈不上偏高。上海/广州 4–6 月清晨 22–24℃
  // 配 85–95% 湿度是常态，用户刷新两次就能看见同一瓶香换一个裁决。
  //
  // 这一段连续化重写的全部动机就是消灭 0.1℃ 级的断崖，结果在湿度轴上新造了一个
  // 0.01℃ 级、影响面更大的。乘上去之后湿度的加成随热负荷一起长起来，
  // 也才对得上下面那句"湿度只做小幅加成"。
  const M = H * clamp01((rh - 60) / 30);
