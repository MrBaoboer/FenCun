...
- 🔔 **发现型钩子** — 主动提醒，而非等你来问：常喷的那瓶今天会翻车（急性天气 / 反季 / 场合预警）、搁置已久的那瓶今天正合适（吃灰提醒）。
...
> 场合权重刻意略高于季节——**「今天去哪儿」比「现在什么季」更该决定喷哪瓶**（急性温度由乘性 W 兜底）。质量先验只作轻推，是为了避免高分香跨场景通吃。
...
- **统一榜单**：主目录与扩展集合并重排、不分区，文本匹配档位优先、同档位按社区投票的主流度。
- **中文化**：香调 / 气味词近全覆盖；香名 **89.0%** 有中文名，剩下 165 款没有可靠中文名，保留英文——错的中文名比英文更糟。原始数据不入仓库，仅提交构建产物；完整管线与数字见 [数据工程](docs/数据工程.md)。
...
两个脚本都把无头浏览器开到站点自己的页面上取材，字体、配色与排版和线上**同源**；截图用的就是演示香柜本身。
