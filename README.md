...
- 🔁 **反馈闭环** — 答一句「今天，刚好吗」，个人偏移按瓶收敛：嫌冲下次少喷，「刚好」记住配置直接复用，高温天的「淡了」归因天气、不冤枉香水。
...
- **分层**：原始 13.2 万款 → 投票数 ≥ 50 筛得 3.67 万款。主目录取热度 **Top 1500 全中文精选**；其余进**扩展集**（索引懒加载 + 64 分片按需取，含 785 款国货白名单）；再搜不到还有**手动记一瓶**兜底——每一瓶都进得来。
- **统一榜单**：主目录与扩展集合并重排、不分区，文本匹配档位优先、同档位按社区投票的主流度——高度匹配的结果永远靠前。
- **中文化**：香调 / 气味词近全覆盖，香名 **89%** 采用官方名或香圈通行名，无可靠中文名的保留英文——错的中文名比英文更糟。原始数据不入仓库，仅提交构建产物；完整管线与数字见 [数据工程](docs/数据工程.md)。
